...
public interface VendaRepository extends JpaRepository<UserOrder, Long> {
        ...
        List<UserOrder> findByTypeStock(Long id_stock);//procurar ordens de venda abertas
        ...
        List<UserOrder> findByCalculo();//Pegando matches
        ...
        int updateRemainingValue(Long id_stock, UserOrder id_order);//Ele atualiza remaining value quando há match
        ...
        List<UserOrder> testando1();

        @Modifying
        @Query(value = "update users_orders  set remaining_value = 0 where id_order=?1 and type = 1 ", nativeQuery = true)
        int updateRemainingValue2(UserOrder id_order);//Ele atualiza remaining value quando há match
        ...
        int RemainingNE(User id_user, Long id_stock);
