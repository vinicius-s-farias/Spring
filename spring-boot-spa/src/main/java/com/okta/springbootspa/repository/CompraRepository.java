...
//Ele atualiza o raimaning value conforme compras são realizadas
    ...
    @Modifying
    @Query(value = "update users set dollar_balance = dollar_balance - ( select uo.remaining_value  * a.price FROM users_orders a, users_orders uo " +
    ...
    @Modifying
    @Query(value = "update users_stocks_balances set volume = volume + (select a.volume - uo.remaining_value " +
            " AS ID FROM users_orders a, users_orders uo " +
            " inner join users u on uo.id_user = u.id " +
            " inner join users_stocks_balances usb on u.id = usb.id_user " +
            " WHERE a.type <> uo.type  and a.id_stock = uo.id_stock and a.id_order= ?1 fetch first 1 rows only ) where id_user = ?2 and id_stock = ?3", nativeQuery = true)
    ...
    @Query(value = " select * from " +
            "                users_orders a, users_orders b where a.type <> b.type and a.remaining_value > b.volume and a.type = 0 and b.type = 1 and a.id_stock = b.id_stock and a.id_order <> b.id_order " +
            "                and a.status = 1 and a.remaining_value <> 0 ", nativeQuery = true)
    List<UserOrder> fyndteste();


    @Query(value = "  select * from " +
            "  users_orders a, users_orders b where a.type <> b.type and  a.remaining_value <= b.volume and a.type = 0 and a.status = 1 and a.id_stock = b.id_stock and a.id_order <> b.id_order order by a.created_on asc ", nativeQuery = true)
    List<UserOrder> findtTeste1();

    @Modifying
    @Query(value = " update users_orders  set remaining_value = ( select a.remaining_value  - uo.volume  from " +
            " users_orders a, users_orders uo inner join users u on uo.id_user = u.id " +
            " where a.type = 0 and a.id_stock = uo.id_stock  and uo.id_order = ?1 and uo.id_user = ?2 order by a.created_on asc fetch first 1 rows only) where id_order=?1 and type = 0", nativeQuery = true)
